                          ...
                          struct flb_config *config)
{
    ...
    if (flb_output_is_threaded(out_ins) == FLB_TRUE) {
        flb_debug("flb_output; flb_output_task_flush;Output plugin is threaded; Dispatch to thread pool");
        ...
        flb_debug("flb_output; flb_output_task_flush;Output plugin is non threaded; ");
        flb_debug("flb_output; flb_output_task_flush;Init coro instance ");
        out_coro = flb_output_coro_create(task,
                                          ...
                                          task->tag_len);
        if (!out_coro) {
            flb_debug("flb_output; flb_output_task_flush;Coro instance initialization unsuccessfull; ");
            return -1;
        }
        flb_debug("flb_output; flb_output_task_flush;Proceed task execution with coro instance ");
        flb_task_users_inc(task);
        flb_coro_resume(out_coro->coro);
        flb_debug("flb_output; flb_output_task_flush;Proceed task execution successfull with coro instance ");
