...
static int prepare_destroy_conn(struct flb_upstream_conn *u_conn)
{
    ...
    flb_debug("flb_upstream.c; prepare_destroy_conn ;[upstream] destroy connection #%i to %s:%i", 
              u_conn->fd, u->tcp_host, u->tcp_port);
...
static struct flb_upstream_conn *create_conn(struct flb_upstream *u)
{
    ...
    now = time(NULL);
    flb_debug("flb_upstream.c; create_conn; Create new Connection");
    ...
    conn->ka_count = 0;
    conn->coro = coro;
    flb_debug("flb_upstream.c; create_conn; Setting up tls connection");
...
struct flb_upstream_conn *flb_upstream_conn_get(struct flb_upstream *u)
{
    ...
    uq = flb_upstream_queue_get(u);
    flb_debug("flb_upstream.c; flb_upstream_conn_get; [upstream] get new connection for %s:%i, net setup:\n"
    ...
    mk_list_foreach_safe(head, tmp, &uq->av_queue) {
        ...
        if (!FLB_EINPROGRESS(err) && err != 0) {
            flb_debug("flb_upstream.c; flb_upstream_conn_get;[upstream] KA connection #%i is in a failed state "
                      "to: %s:%i, cleaning up",
                      conn->fd, u->tcp_host, u->tcp_port);
            prepare_destroy_conn_safe(conn);
            conn = NULL;
        ...
        /* Connect timeout */
        conn->ts_assigned = time(NULL);
        flb_debug("flb_upstream.c; flb_upstream_conn_get;[upstream] KA connection #%i to %s:%i has been assigned (recycled)",
                  conn->fd, u->tcp_host, u->tcp_port);
        /*
