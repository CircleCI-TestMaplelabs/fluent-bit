                                      ...
                                      int ch_parent, flb_pipefd_t fd)
{
    ...
    flb_debug("flb_output_threads.c; handle_output_event; Output flush callback has finished");
    ...
    while (running) {
        ...
        mk_event_foreach(event, th_ins->evl) {
            ...
            if (event->type == FLB_ENGINE_EV_CORE) {
                ...
                flb_plg_debug(th_ins->ins, "thread worker #%i starting...",
                                 thread_id);
                ...
                out_coro = flb_output_coro_create(task,
                                                  ...
                                                  task->tag_len);
                if (!out_coro) {
                    flb_plg_debug(th_ins->ins, "thread worker #%i Unsuccessfull...",thread_id);
                    continue;
                }
                flb_plg_debug(th_ins->ins, "thread worker #%i Done...",thread_id);
