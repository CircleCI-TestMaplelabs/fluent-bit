...
static inline int handle_output_event(flb_pipefd_t fd, struct flb_config *config)
{
    ...
    if (flb_output_is_threaded(ins) == FLB_FALSE) {
        flb_debug("flb_engine.c; handle_output_event; Output flush callback has finished");
        flb_output_flush_finished(config, out_id);
    }

    /* A thread has finished, delete it */
    if (ret == FLB_OK) {
        flb_debug("flb_engine.c; handle_output_event; Event thread set to be deleted");
        /* Update metrics */
#ifdef FLB_HAVE_METRICS
        if (ins->metrics) {
        ...
        flb_debug("flb_engine.c; handle_output_event; Event thread deleted");
    }
    else if (ret == FLB_RETRY) {
        ...
        flb_debug("flb_engine.c; handle_output_event; Event thread retry created");
                                              ...
                                              struct flb_config *config)
{
    ...
    if (mask & MK_EVENT_READ) {
        ...
        if (config->flush_fd == fd) {
            ...
            flb_debug("[engine] Received a flush event");
            flb_engine_flush(config, NULL);
            return 0;
        }
...
int flb_engine_start(struct flb_config *config)
{
    ...
    flb_debug("Start Event loop successfull");
    /* Start the Storage engine */
    ret = flb_storage_create(config);
    if (ret == -1) {
        return -1;
    }
    flb_debug("Start Storage successfull");
    /* Init Metrics engine */
    cmt_initialize();
    ...
    flb_debug("Init input plugins successfull");
    /* Initialize filter plugins */
    ret = flb_filter_init_all(config);
    if (ret == -1) {
        return -1;
    }
    flb_debug("Init Filter plugins successfull");

    /* Inputs pre-run */
    flb_input_pre_run_all(config);
    flb_debug("Init input plugins pre-run successfull");
    ...
    flb_debug("Init Output plugins successfull");
    ...
    flb_debug("Init Collector plugins successfull");
    ...
    flb_debug("[engine] Engine successfully configured");
        ...
        mk_event_foreach(event, evl) {
            if (event->type == FLB_ENGINE_EV_CORE) {
                ...
                if (co) {
                    ...
                    flb_debug("[engine] resuming coroutine=%p", co);
                    flb_coro_resume(co);
                }
            }
                 ...
                 * status.
                 */
                flb_debug("flb_engine.c; flb_engine_start; Event received");
